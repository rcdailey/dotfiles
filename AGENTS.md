...
- `exact_agents/` - Custom agent definitions (target: `~/.config/opencode/agents/`). The primary
  agents (`architect`, `build`, `dispatch`) are chezmoi templates that include a shared partial via
  `{{ template "opencode-primary-shared.md" . }}`. `dispatch` is `hidden: true` (headless ticket
  work only; zero-ask permission surface because headless runs auto-reject permission asks). The
  partial holds directives that apply to every primary agent but MUST NOT leak to subagents.
  OpenCode loads AGENTS.md into every agent context regardless of mode (verified against source; no
  mode-based filter exists), so shared primary-only content cannot live in AGENTS.md and the
  template partial is the workaround.
