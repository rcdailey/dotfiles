...
:   This is used by the \_all\_matches completer to decide if the string
    ...
    The default value for this style is ‘\_expand \_old\_list \_correct
...
:   This is used by the \_oldlist completer. If it is set to ‘always’,
    ...
    For example, suppose you type ^Xc to use the \_correct\_word
...
If the first argument to \_main\_complete is a single hyphen, the
...
:   Complete words from the shell’s command history. This completer
    ...
    \_history\_complete\_word bindable command, see
    [Bindable Commands](#Bindable-Commands)
    and
...
:   This completer was written as simple example function to show how menu
    ...
    MENU\_COMPLETE option and does not work with the other menu
...
:   This completer can be used to try completion with the suffix (everything
    ...
    COMPLETE\_IN\_WORD option is set; otherwise, the cursor will
...
:   This completer behaves similarly to the \_expand completer but
    ...
    \_user\_expand in addition to other styles handled more generally by
...
|  |  |  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [ [<<](#Completion-System "Beginning of this chapter or previous chapter") ] | [ [<](#Control-Functions "Previous section in reading order") ] | [ [Up](#Completion-System "Up section") ] | [ [>](#Completion-Functions "Next section in reading order") ] | [ [>>](Completion-Using-compctl.html#Completion-Using-compctl "Next chapter") ] |  |  |  |  | [[Top](index.html#Top "Cover (top) of document")] | [[Contents](zsh_toc.html#SEC_Contents "Table of contents")] | [[Index](Concept-Index.html#Concept-Index "Index")] | [ [?](zsh_abt.html#SEC_About "About (help)") ] |
...
\_bash\_completions
:   This function is used by two widgets, \_bash\_complete-word and
    \_bash\_list-choices. It exists to provide compatibility with
...
:   This function can be used as a completer and as a bindable command.
    ...
    case, if the word is not the name of an alias, \_expand\_alias tries
    ...
    \_expand\_alias once more will expand the now-complete alias name.
...
:   This function is not defined as a widget and not bound by
    ...
    \_main\_complete as the list of completers in place of those defined by
    the completer style.
...
:   Prompt the user for a string, and use that to perform completion on the
    ...
    Once a string has been read, the next call to \_read\_comp will use the
    existing string instead of reading a new one. To force a new string to be
    read, call \_read\_comp with a numeric argument.
...
:   Unlike other commands listed here, this must be created as a normal ZLE
    ...
    the same format that the \_complete\_help widget displays for
    contextual completion.

    If the widget’s name contains debug, for example if it is created
    as ‘zle -N \_complete\_debug\_generic \_complete\_help\_generic’, it
    will read and execute the keystring for a generic widget as before,
    but then generate debugging information as done by \_complete\_debug
    ...
    ZSH\_TRACE\_GENERIC\_WIDGET which is read by \_generic. Unsetting
...
|  |  |  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [ [<<](#Completion-System "Beginning of this chapter or previous chapter") ] | [ [<](#Bindable-Commands "Previous section in reading order") ] | [ [Up](#Completion-System "Up section") ] | [ [>](#Completion-System-Variables "Next section in reading order") ] | [ [>>](Completion-Using-compctl.html#Completion-Using-compctl "Next chapter") ] |  |  |  |  | [[Top](index.html#Top "Cover (top) of document")] | [[Contents](zsh_toc.html#SEC_Contents "Table of contents")] | [[Index](Concept-Index.html#Concept-Index "Index")] | [ [?](zsh_abt.html#SEC_About "About (help)") ] |
...
:   This function completes external commands as absolute paths (unlike
    \_command\_names -e which completes their basenames). It takes no
    arguments.

\_all\_labels [ -x ] [ -12VJ ] tag name descr [ command arg ... ]
:   This is a convenient interface to the \_next\_label function below,
    implementing the loop shown in the \_next\_label example. The
    ...
    command is called. This allows \_all\_labels to be used in almost all
    ...
    those generated by \_all\_labels.
...
:   This function is useful in simple cases where multiple tags are available.
    ...
    * The ‘((a\:bar b\:baz))’ form does not need
    ...
    Like \_arguments, this function uses \_all\_labels to execute
...
:   This function can be used to give a complete specification for completion
    ...
    *Options Overview*
    ...
    not -sw. The options are followed by specs that describe options and
    ...
    -w
    :   In combination with -s, allow option stacking
    ...
    -C
    :   Modify the curcontext parameter for an action of the form ‘->state’.
    ...
    :   Use the match specification matchspec for completing option names and values.
        The default matchspec allows partial word completion after ‘\_’ and
    ...
    -0
    :   When populating values of the ‘opt\_args’ associative array, don’t
    ...
    *specs: overview*
    ...
    specific contexts: on the first call ‘\_arguments $global\_options’ is
    used, and on subsequent calls ‘\_arguments !$^global\_options’.
    ...
    *specs: actions*
    ...
    \_all\_labels function to process all tag labels. No special handling
    ...
    :   In this form, \_arguments processes the arguments and options and then
        ...
        action is to be performed. The elements of the array state\_descr are
        ...
        In addition to $state and $state\_descr, \_arguments also
        sets the global
        parameters ‘context’, ‘line’ and ‘opt\_args’ as described
    ...
    ‘opt\_args’ with option names as keys and their arguments as
    ...
    using an optspec of the form ‘\*optspec’), they are joined with
    ...
    *Grouping Options*
    ...
    ‘(name)’ then only one value from that group
    ...
    associative array ‘opt\_args’ with ’group-option’
    ...
    *Deriving spec forms from the help output*
    ...
    resetting all the locale categories (except for LC\_CTYPE) to ‘C’.
...
:   If a function name exists, it is called with the arguments
    ...
    The return status of \_call\_function itself is zero if the function
...
:   This function provides a mechanism for the user to override the use of an
    ...
    execute. The strings from the call to \_call\_program, or from the
    ...
    categories (except for LC\_CTYPE) are reset to ‘C’ by calling the
    utility function \_comp\_locale (see below). If the option ‘-l’ is
    ...
    gain-privileges style is set to true, \_call\_program will make
...
:   This function completes words that are valid at command position: names of
    ...
    flag, only hashed commands are completed. The - flag is ignored.

\_comp\_locale
:   This function resets all the locale categories other than LC\_CTYPE to
    ‘C’ so that the output from external commands can be easily analyzed by
    the completion system. LC\_CTYPE retains the current value (taking
    LC\_ALL and LANG into account), ensuring that non-ASCII characters
...
:   This function associates completions with descriptions.
    ...
    \_describe uses the \_all\_labels function to generate the matches, so
...
:   This function is not to be confused with the previous one; it is used as
    ...
    every appearance of ‘%char’ will be replaced by string.
...
:   Complete a list of directory names separated by colons
    ...
    -s sep
    :   Use sep as separator between items.
...
:   Complete email addresses. Addresses are provided by plugins.
    ...
    :   Complete bare localhost@domain.tld addresses, without a name part or
        a comment.
        Without this option, RFC822 ‘Firstname Lastname <address>’
    ...
    \_email-local (completes user@hostname Unix addresses),
    ...
    Addresses from the \_email-foo plugin are added under the
    tag ‘email-foo’.
    ...
    *Writing plugins*
    ...
    set the $reply array to a list of ‘alias:address’ elements and return 300.
    New plugins will be picked up and run automatically.

\_files
:   The function \_files is a wrapper around \_path\_files. It supports
    ...
    style. \_files should therefore be preferred over \_path\_files in
    most cases.

    This function accepts the full set of options allowed by
    \_path\_files, described below.
...
:   The argument sep is a separator character.
    ...
    ‘\_multi\_parts / patharray’ to complete partial file paths
    from the given array of complete file paths.

    The -i option causes \_multi\_parts to insert a unique match even
...
:   This function is used to implement the loop over different tag
    ...
    over \_next\_label handles any labels defined for the tag in the
    tag-order style.
...
:   This is the standard function called to handle completion outside
    ...
    -p precommand
    :   Append precommand to the list of precommands. This option should be
        used in nearly all cases in which -P is not applicable.
...
:   This can be used to complete the names of shell options. It provides a
    ...
    ‘noglob’, ‘NO\_GLOB’ are all completed). Any arguments
...
:   These functions complete only set or unset options, with the same
    ...
    Note that you need to uncomment a few lines in the \_main\_complete
...
:   This function is used throughout the completion system
    ...
    Both \_path\_files and \_files also accept the following options
    which are passed to compadd: ‘-J’, ‘-V’,
    ‘-1’, ‘-2’, ‘-n’, ‘-X’, ‘-M’, ‘-P’, ‘-S’,
    ‘-q’, ‘-r’, and ‘-R’.

    Finally, the \_path\_files function uses the styles expand,
...
:   This function is used to resolve situations where a single command name
    ...
    label=pattern; if the output of ‘command arg
    ...
    The results are also cached in the \_cmd\_variant associative array
...
:   This function generates a completion function name which matches
    ...
    described below. After running \_regex\_arguments, the function
    ...
    :   Parentheses may be used to groups specs; note each parenthesis
        is a single argument to \_regex\_arguments.
    ...
    The function \_regex\_words can be used as a helper function to
    ...
    any argument. In this case \_regex\_words could be used to generate
...
:   This function can be used to generate arguments for the
    \_regex\_arguments command which may be inserted at any point where
    ...
    The result of the processing by \_regex\_words is placed in the array
    ...
    array mydb\_add\_cmds, quite possibly by a previous call to
    \_regex\_words.
...
:   This function is called to decide whether a tag already registered by a
    ...
    If command is given, the \_all\_labels function will be called
...
:   This function retrieves completion information from the file given by
    cache\_identifier, stored in a directory specified by the
...
:   This function is passed alternating arrays and separators as arguments.
    ...
    ‘hosts=(ftp news)’ the call ‘\_sep\_parts ’(foo bar)’ @ hosts’ will
...
:   This function, together with \_retrieve\_cache and
    \_cache\_invalid, implements a caching layer which can be used
    ...
    data can then be retrieved quickly from that file via \_retrieve\_cache,
    even in different instances of the shell.

    The cache\_identifier specifies the file which the data should be
    ...
    The completion function may avoid calling \_retrieve\_cache when it
    already has the completion data available as parameters.
    However, in that case it should
    call \_cache\_invalid to check whether the data in the parameters and
    in the cache are still valid.

    See the \_perl\_modules completion function for a simple example of
...
:   This is used to complete arbitrary keywords (values) and their arguments,
    ...
    The first non-option argument, desc, is used as a string to print as a
    ...
    The associative array val\_args is used to report values and their
    arguments; this works similarly to the opt\_args associative array
    used by \_arguments. Hence the function calling \_values should
    declare the local parameters state, state\_descr, line,
    context and val\_args:
    ...
    the state and state\_descr are scalars rather than arrays.
...
:   In many contexts, completion can only generate one particular set of
    ...
    See also the use of \_wanted in the example function in
    [Dynamic named directories](Expansion.html#Dynamic-named-directories).
...
:   This function completes names of zle widgets (see
    [Zle Widgets](Zsh-Line-Editor.html#Zle-Widgets)). The pattern, if present, is matched against values of the $widgets
    special parameter, documented in
    [The zsh/zleparameter Module](Zsh-Modules.html#The-zsh_002fzleparameter-Module).

---

|  |  |  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [ [<<](#Completion-System "Beginning of this chapter or previous chapter") ] | [ [<](#Completion-Functions "Previous section in reading order") ] | [ [Up](#Completion-System "Up section") ] | [ [>](#Completion-Directories "Next section in reading order") ] | [ [>>](Completion-Using-compctl.html#Completion-Using-compctl "Next chapter") ] |  |  |  |  | [[Top](index.html#Top "Cover (top) of document")] | [[Contents](zsh_toc.html#SEC_Contents "Table of contents")] | [[Index](Concept-Index.html#Concept-Index "Index")] | [ [?](zsh_abt.html#SEC_About "About (help)") ] |

## 20.7 Completion System Variables

There are some standard variables, initialised by the \_main\_complete
...
:   The completion system uses setopt to set a number of options. This
    ...
    in the \_comp\_caller\_options associative array. Option names, spelled
    in lowercase without underscores, are mapped to one or other of the
    strings ‘on’ and ‘off’.

\_comp\_priv\_prefix
:   Completion functions such as \_sudo can set the \_comp\_priv\_prefix
    array to a command prefix that may then be used by \_call\_program to
    match the privileges when calling programs to generate matches.

Two more features are offered by the \_main\_complete function. The
...
|  |  |  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [ [<<](#Completion-System "Beginning of this chapter or previous chapter") ] | [ [<](#Completion-System-Variables "Previous section in reading order") ] | [ [Up](#Completion-System "Up section") ] | [ [>](Completion-Using-compctl.html#Completion-Using-compctl "Next section in reading order") ] | [ [>>](Completion-Using-compctl.html#Completion-Using-compctl "Next chapter") ] |  |  |  |  | [[Top](index.html#Top "Cover (top) of document")] | [[Contents](zsh_toc.html#SEC_Contents "Table of contents")] | [[Index](Concept-Index.html#Concept-Index "Index")] | [ [?](zsh_abt.html#SEC_About "About (help)") ] |
...
|  |  |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [ [<<](#Completion-System "Beginning of this chapter or previous chapter") ] | [ [>>](Completion-Using-compctl.html#Completion-Using-compctl "Next chapter") ] |  |  |  |  |  | [[Top](index.html#Top "Cover (top) of document")] | [[Contents](zsh_toc.html#SEC_Contents "Table of contents")] | [[Index](Concept-Index.html#Concept-Index "Index")] | [ [?](zsh_abt.html#SEC_About "About (help)") ] |
