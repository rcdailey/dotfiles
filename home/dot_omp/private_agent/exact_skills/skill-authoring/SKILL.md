...
Skills are on-demand procedures. AGENTS.md holds primary-only invariants, RULES.md holds invariants
shared with task subagents, and documentation holds reference material. Permissions and checks
...
1. `name` and `description`: metadata shown in OMP's skill catalog.
...
OMP discovers project skills under `.omp/skills/<name>/SKILL.md` and user skills under
`~/.omp/agent/skills/<name>/SKILL.md`. It also imports compatible Agent Skills locations.
...
- OMP also recognizes `globs`, `alwaysApply`, `hide`, and `disableModelInvocation`.
- Unknown frontmatter is preserved as metadata; do not invent behavioral fields.
...
consequential error. Put primary-only registration in AGENTS.md and universal registration in
RULES.md.
...
- Verify frontmatter against the current OMP documentation.
- Confirm the name matches the directory and is unique across discovery locations.
- Remove instructions inherited from RULES.md or another skill.
