"""Custom Click classes: compact recursive help and repo inference."""

from __future__ import annotations

import click

from gh_review._gh import current_repo


class RepoCommand(click.Command):
    """Command whose leading REPO argument defaults to the current repository.

    The repo is inferred when the first argument is missing or is not an
    ``owner/name`` pair, so ``gh-review view 3128`` works inside a checkout.
    """

    def parse_args(self, ctx: click.Context, args: list[str]) -> list[str]:
        if not args or (not args[0].startswith("-") and "/" not in args[0]):
            args = [current_repo(), *args]
