...
def test_bot_substring_without_brackets():
    assert is_bot("botperson") is False


def test_is_bot_empty():
    assert is_bot("") is False


def test_graphql_typename_bot():
    assert is_bot("linear", "Bot") is True


def test_graphql_typename_user():
    assert is_bot("linear", "User") is False


def test_graphql_typename_empty():
    assert is_bot("coderabbitai", "") is False


def test_graphql_typename_bot_without_bracket_suffix():
    assert is_bot("coderabbitai", "Bot") is True


def test_strips_details_blocks():
    body = (
        "Main finding.\n"
...
def test_strips_decorative_separators():
    body = "Content above\n---\nContent below"
    result = sanitize_bot_body(body)
...
def test_strips_nested_details():
    body = (
        "Finding.\n"
...
def test_preserves_plain_content():
...
def test_strips_mermaid_blocks():
    body = "## Summary\n\n```mermaid\nflowchart LR\n  A --> B\n```\n\nDetails here."
    result = sanitize_bot_body(body)
...
def test_strips_inline_html_keeps_text():
...
def test_strips_badge_links():
    body = (
        "Status: [![Build](https://img.shields.io/badge.svg)](https://example.com)\nReal content."
    )
    result = sanitize_bot_body(body)
...
def test_strips_image_markdown():
...
def test_cloudflare_deploy_comment():
...
def test_sourcery_reviewer_guide():
...
def test_markdown_table_with_html_in_cells():
...
def test_zero_max_returns_original():
    assert truncate_body("hello", 0) == "hello"


def test_negative_max_returns_original():
